...
async function main() {
  ...
  await fs.mkdir(path.join(root, "labs", "demo"), { recursive: true });
  await fs.writeFile(path.join(root, "labs", "demo", "index.html"), "<h1>lab preview</h1>");
  const tournament = {
  ...
  const client = {
    ...
    async peekTournament() {
      return { data: tournament, source: "cache", stale: false };
    },
  ...
  try {
    ...
    const preview = await fetch(`${base}/labs/demo/`).then((response) => response.text());
    assert.ok(preview.includes("lab preview"));
...
async function staleCacheReturnsImmediately() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "worldcup-server-stale-test-"));
  await fs.writeFile(path.join(root, "index.html"), "<h1>test</h1>");
  const tournament = {
    meta: { syncedAt: "2026-06-28T00:00:00.000Z", matchCount: 104 },
    matches: [],
  ...
  const client = {
    constants: { refreshMs: 60 * 60 * 1000 },
    async peekTournament() {
      return { data: tournament, source: "stale-cache", stale: true };
    },
    async loadTournament() {
      refreshCalls += 1;
      await new Promise((resolve) => setTimeout(resolve, 200));
      return { data: tournament, source: "stale-cache", stale: true, error: "offline" };
    },
  ...
  try {
    ...
    assert.ok(elapsed < 100, `stale response took ${elapsed}ms`);
    await new Promise((resolve) => setTimeout(resolve, 240));
    assert.equal(refreshCalls, 1);
    console.log("PASS stale tournament cache returns before background refresh finishes");
  } finally {
    await new Promise((resolve) => server.close(resolve));
  }
}

main().then(staleCacheReturnsImmediately).catch((error) => {
